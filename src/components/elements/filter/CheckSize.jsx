...
import { setSelectedSizes } from "@/stores/Cart";

const CheckSize = () => {
  const dispatch = useDispatch();
  const [selectedSizes, setSelectedSizesState] = useState({
    M: false,
    L: false,
    XL: false,
  });
  ...
  const handleCheckboxChange = (size) => {
    setSelectedSizesState((prevSizes) => ({
      ...prevSizes,
      [size]: !prevSizes[size],
    }));
  };
  ...
  const handleApply = () => {
    ...
    dispatch(setSelectedSizes(selected));
  };
