...
const ModalSearch = ({ isVisible, onClose }) => {
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.div
          className="fixed inset-0 z-50 flex justify-center px-4 bg-black bg-opacity-80 sm:px-0"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          <div className="relative w-full py-10 rounded-lg shadow-lg top-40 sm:w-1/2 ">
            <div className="flex gap-2">
              <div className="flex items-center w-full px-4 py-4 overflow-hidden border-2 border-zinc-700 rounded-xl bg-black/80 ">
                <Input
